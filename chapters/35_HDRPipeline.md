...
**Formula** (Narkowicz 2016 approximation):
$$L_{out} = \frac{L_{in} \cdot (a \cdot L_{in} + b)}{L_{in} \cdot (c \cdot L_{in} + d) + e}$$
...
**Characteristics**:
- **Pros**: Film-like contrast, industry-standard, excellent highlight rolloff
- **Cons**: More complex, not as intuitive to tune
- **Visual**: Cinematic, pleasant contrast curve, slightly desaturated highlights
...
> [!IMPORTANT]
> ACES is the **recommended default** for production. It's been battle-tested in thousands of games and provides consistently good results across different scenes.
...
**Expected Match**:
- ACES operator should look very similar
- Exposure control should behave the same

**Differences**:
- Unreal uses full ACES RRT+ODT (we use Narkowicz approximation)
